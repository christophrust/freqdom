...
#' DFT = fourier_transform(tdX, freq= pi*-1000:1000/1000)
#' @useDynLib freqdom
#' @export
fourier_transform = function(A,freq=pi*-100:100/100){
  if (!is.timedom(A))
  ...
  s <- .Call("fourier_transform",
             as.vector(A$operators),
             as.integer(nbasisX),
             as.integer(nbasisY),
             as.numeric(thetas),
  ...
  S = array(
    s,
    dim = c(nbasisX, nbasisY, length(thetas))
  )
